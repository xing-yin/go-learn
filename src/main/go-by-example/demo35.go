...
/**
速率限制
速率限制是一个重要的控制服务资源利用和质量的途径。Go 通过 Go 协程、通道和打点器优雅地支持了速率限制。
*/
func main() {
	...
	bursytyLimiter := make(chan time.Time, 3)

	// 想将通道填充需要临时改变3次的值，做好准备
	for i := 0; i < 3; i++ {
		bursytyLimiter <- time.Now()
	}

	// 每 200 ms 我们将添加一个新的值到 burstyLimiter中，直到达到 3 个的限制
	go func() {
		for t := range time.Tick(time.Microsecond * 2000000) {
			bursytyLimiter <- t
		}
	}()

	// 现在模拟超过 5 个的接入请求。它们中刚开始的 3 个将由于受 burstyLimiter 的“脉冲”影响
	bursytyRequests := make(chan int, 5)
	for i := 1; i <= 5; i++ {
		bursytyRequests <- i
	}
	close(bursytyRequests)
	for req := range bursytyRequests {
		<-bursytyLimiter
		fmt.Println("request", req, time.Now())
	}
